...
const PhotoGalleryInAboutPage = ({ data, itemImageClicked }) => {
    ...
    const renderPhotos = (parameter) => {
        ...
        const tempData =
            parameter === 'first'
                ? dataForRender.slice(0, 3)
                : dataForRender.slice(3);
        return tempData.map((item, index) => {
            let classNameLi =
                (index === 2 && parameter === 'first') ||
                (index === 0 && parameter === 'second')
                    ? 'photo-gallery-large'
                    : 'photo-gallery-small';
            const isLarge =
                classNameLi === 'photo-gallery-large' ? true : false;
            let isFirstLarge =
                isLarge && parameter === 'first' ? 'is-first-large' : '';
            let isSecondSmall =
                !isLarge && parameter === 'second' ? 'is-second-small' : '';
            ...
            <div className="col-8 photo-gallery-content-wrapper">
                <div className="photo-gallery-title-and-buttons-wrapper">
                    ...
                    <div className="photo-gallary-buttons-wrapper">
                        <div
                            className="photo-gallary-button"
